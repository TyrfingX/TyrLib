...
import com.tyrfing.games.tyrlib3.graphics.animation.Animation;
import com.tyrfing.games.tyrlib3.graphics.animation.AnimationFrame;
import com.tyrfing.games.tyrlib3.graphics.animation.Bone;
import com.tyrfing.games.tyrlib3.graphics.animation.Skeleton;
...
import com.tyrfing.games.tyrlib3.graphics.renderables.SubEntity;
import com.tyrfing.games.tyrlib3.graphics.renderer.Mesh;
import com.tyrfing.games.tyrlib3.graphics.renderer.Texture;
...
public class IQMEntityFactory implements IEntityFactory {
	...
	public class SubEntityPrototype {
		Mesh mesh;
				...
				case 2: // This is a normal
					...
					for (int k = 0; k < subEntity.countVertices; ++k) {
						...
						if (light != null) {
							...
							float diffuse = Math.max(Vector3F.dot(x,y,z,lightVector[0],lightVector[1],lightVector[2]), 0);
			...
			for (int i = 0; i < header.num_anims; ++i) {
				...
				for (int j = 0; j < animData.animationFrames.length; ++j) {
					...
					for (int k = 0; k < header.num_poses; ++k) {
						...
						Vector3F translate = new Vector3F(p.channeloffset[0], p.channeloffset[1], p.channeloffset[2]);
