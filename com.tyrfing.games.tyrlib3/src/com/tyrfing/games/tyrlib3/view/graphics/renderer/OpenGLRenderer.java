...
import com.tyrfing.games.tyrlib3.view.graphics.compositors.Composit;
import com.tyrfing.games.tyrlib3.view.graphics.compositors.Compositor;
...
public abstract class OpenGLRenderer extends GameLoop {
	...
	class RenderChannel {
		...
		public RenderChannel(int priority) {
			octree = new Octree(5, 20, new Vector3F(), 2000);
		...
		if (!skipRendering) {
			...
			if (shadowsEnabled) {
				...
				if (shadowDistanceIndex != -1) {
					...
					Vector3F look = new Vector3F(lightDir[0], lightDir[1], lightDir[2]);
					Vector3F right = look.cross(new Vector3F(0,0,1));
					Vector3F up = right.cross(look);
					up.normalize();
					
					Vector3F camPos = new Vector3F(camera.getAbsolutePos());
	...
	public void render() {
    	if (rendering) {
		    ...
		    for (int i = 0; i < sceneManager.getLightCount(); ++i) {
		    	...
		    	light.update(camera.getViewMatrix());
		    }
		    ...
		    Matrix.multiplyMM(vpMatrix, 0, viewport.getProjectionMatrix(), 0, camera.getViewMatrix(), 0);
