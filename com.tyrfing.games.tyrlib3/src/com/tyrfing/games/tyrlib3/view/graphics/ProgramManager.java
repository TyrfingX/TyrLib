...
import com.tyrfing.games.tyrlib3.model.files.FileReader;
...
public class ProgramManager {
	...
	public Program createProgram(String programName, int vertexShaderResId, int fragmentShaderResId, String[] bindAttributes, PreprocessorOptions options) {
		...
		program.setBindAttributes(bindAttributes);
		return program;
	}
	...
	public void recreateAll() {
		Program.setInUse(null);
		
		for (String programName : programs.keySet()) {
			
			Program program = programs.get(programName);
			
			int vertexShader = ShaderManager.getInstance().loadShader(programName + "_VS", TyrGL.GL_VERTEX_SHADER, program.getVertexShader());
			int fragmentShader = ShaderManager.getInstance().loadShader(programName + "_FS", TyrGL.GL_FRAGMENT_SHADER, program.getFragmentShader());
	        ...
	        if (program.getBindAttributes() != null) {
	        	for (int i = 0; i < program.getBindAttributes().length; ++i) {
	        		TyrGL.glBindAttribLocation(programHandle, i, program.getBindAttributes()[i]);
	        	}
	        }
