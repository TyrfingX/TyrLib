...
import com.tyrfing.games.tyrlib3.graphics.renderables.BoundingBox;
...
public class OctreeNode extends BoundedSceneObject {
	...
	public OctreeNode(int minimumObjectsPerNode, int maximumObjectsPerNode, Vector3F center, float dimension) {
		...
		boundingBox = new AABB(	new Vector3F(-dimension/2+center.x, -dimension/2+center.y, -dimension/2+center.z),
								new Vector3F(dimension/2+center.x, dimension/2+center.y, dimension/2+center.z));
	...
	public void addObject(BoundedSceneObject sceneObject) {
		...
		if (objects.size() <= maximumObjectsPerNode && parentOctree == null && children == null) {
			...
			if (children == null && parentOctree == null) {
				Vector3F pos = sceneObject.getAbsolutePos();
				if (pos != null) {
		...
		if (boundingBox.containsAABB(aabb)) {
			...
			if (parentOctree != null) {
				...
				Vector3F objectCenter = aabb.getCenter();
				Vector3F vectorToObject = boundingBox.getCenter().vectorTo(objectCenter);
				...
				Vector3F parentCenter = new Vector3F(	center.x + childCenterOffsets[0] * dimension/2,
				...
				for (int i = 1; i < CHILDREN_PER_NODE; ++i) {
					Vector3F corner = new Vector3F(	center.x + childCenterOffsets[i*3] * dimension/2,
				...
				for (int i = 0; i < CHILDREN_PER_NODE; ++i) {
					if (i != octant) {
						Vector3F childCenter = new Vector3F(	parentCenter.x + childCenterOffsets[i*3] * dimension/2,
	...
	private void split() {
		...
		for (int i = 0; i < CHILDREN_PER_NODE; ++i) {
			Vector3F childCenter = new Vector3F(	center.x + childCenterOffsets[i*3] * dimension/4,
