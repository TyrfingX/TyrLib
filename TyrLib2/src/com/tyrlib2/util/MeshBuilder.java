...
import com.tyrlib2.graphics.renderables.Quad;
...
public class MeshBuilder {
	...
	public void addQuad(Vector3[] points, Color color) {
		addQuad(points, color, 1, 1);
	}
	
	public void addQuad(Vector3[] points, Color color, float repeatX, float repeatY) {
	 ...
	 * @param points
	 */
	public void addQuad(Vector3[] points, Color color, float repeatX, float repeatY, float[][] textureWeights) {
		...
		if (layout.hasVertexAttribute(VertexLayout.NORMAL)) {
			Vector3 dir1 = points[0].vectorTo(points[1]);
			dir1.normalize();
			Vector3 dir2 = points[0].vectorTo(points[3]);
			dir2.normalize();
